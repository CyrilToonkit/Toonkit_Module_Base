...
def makeHierarchy(iterations=10, mode=0, expanding = 10):
    parentObj = None
    for i in range(iterations):
        loc = pc.spaceLocator(name="Loc%i" % i)
        pc.setAttr(loc + ".tx", (float(i) / float(iterations)) * expanding)
        if parentObj != None:
            if mode == 0:
                pc.parent(loc, parentObj)
            else:
                pc.parentConstraint(parentObj,loc,maintainOffset=True )
        parentObj = loc

#64 base objs
#pc.select("Char_Norman_1", hierarchy=True, replace=True)
#print "Hierarchy " + str(len(pc.ls(sl=True)))
...
def countObjects():
    ...
    for tran in trans:
        if tran.getShape() == None:
            groups.append(tran)
    print "Objects " + str(len(pc.ls()) - 64)
    print " - transforms " + str(len(trans))
    print " - groups " + str(len(groups))
    print " - locators " + str(len(pc.ls(type="locator")))
    print " - joints " + str(len(pc.ls(type="joint")))
    print " - curves " + str(len(pc.ls(type="nurbsCurve")))
...
def traceHierarchy():
    ...
    print "Average depth " + str(float(depths) / count)
    print "Biggest depth " + str(biggestDepth)
...
def countConstraints():
    ...
    print "constraints " + str(len(cons))
    print " - parentConstraints " + str(parentConstraints)
    print " - orientConstraints " + str(orientConstraints)
    print " - aimConstraints " + str(aimConstraints)
    print " - scaleConstraints " + str(scaleConstraints)
    print " - pointConstraints " + str(pointConstraints)
    print " - poleVectorConstraints " + str(poleVectorConstraints)
    print " - motionPaths " + str(motionPaths)
...
def countUtilities():
    ...
    print "utility nodes " + str(len(utils))
...
#rslt = tkc.benchIt(importFile, "Z:\\Toonkit\\OSCAR\\Projects\\Toonkit\\Assets\\Nerd\\AN\\Setup_C_Nerd_AN_FullNodalPoses.mb")
#rslt = tkc.benchIt(importFile, "Z:\\Toonkit\\Veille\\Mery_v3.4_for_2013\\Mery_v3.4_for_2013.mb")
#rslt = tkc.benchIt(importFile, "Z:\\Toonkit\\Veille\\Zoma\\zoma_hi_base_vanilla_rig_v6.mb")
#rslt = tkc.benchIt(importFile, "Z:\\Toonkit\\Veille\\MorpheusRigPack\\Rigs\\morpheusRig_v01_00.mb")
#rslt = tkc.benchIt(importFile, "Z:\\Toonkit\\Veille\\Norman_v010\\Char_Norman_A1_v010.mb")
...
#setAnim(pc.ls(sl=True)[0])

def evaluate(nFrames=100):
    cTime = 0
    for f in range(nFrames):
        cTime += 1
...
#print 1000.0 / tkc.benchIt(evaluate, 1000)[0]



def importFile(path):
    pmsys.importFile(path)
...
def startsWith(inStr, inStart):
    print inStr[:len(inStart)]
    return inStr[:len(inStart)] == inStart

root = pc.ls(sl=True)[0]
children = tkc.getChildren(root, True)

for child in children:
    props = tkc.getProperties(child)
    ...
    for prop in props:
        ...
        if endsWith(prop.name(), "ElementInfo"):
            pc.setAttr(prop.name() + ".nodeState", 2)
